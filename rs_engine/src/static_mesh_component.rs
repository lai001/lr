...
use crate::content::content_file_type::*;
...
impl Component for StaticMeshComponent {
    ...
    fn initialize(
        ...
        let static_mesh_resource = if let Some(static_mesh) = &self.static_mesh
        ...
        if let Some(url) = &self.static_mesh {
            if static_mesh_resource.is_none() {
        ...
        let parament_resource = if let Some(material) = &material {
            let material = material.borrow();
            Self::create_parament_resource(engine, &material)
        } else {
            None
        };

        let (draw_object, mesh, aabb) = if let Some(find_static_mesh) = static_mesh_resource {
...
impl StaticMeshComponent {
    ...
    pub fn set_static_mesh_url(
        ...
        if let Some(static_mesh) = &self.static_mesh {
            if let Some(mesh) = find_content_by_type_ref_map::<
                crate::content::static_mesh::StaticMesh,
            >(files, static_mesh)
            {
        ...
        let mut existing_material: Option<TypedContent<crate::content::material::Material>> = None;
