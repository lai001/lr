...
enum RenderMessage {
    LoopDestroyed,
    RedrawRequested(Option<egui::FullOutput>),
...
struct SpawnRenderThreadResult {
    owned_thread_sender: std::sync::mpsc::SyncSender<RenderMessage>,
    owned_thread_receiver: std::sync::mpsc::Receiver<i32>,
}

struct SpawnAudioThreadResult {
    owned_thread_sender: std::sync::mpsc::Sender<AudioMessage>,
    owned_thread_receiver: std::sync::mpsc::Receiver<i32>,
}
...
impl ExampleApplication {
    ...
    fn spawn_audio_thread() -> SpawnAudioThreadResult {
        let (owned_thread_sender, audio_thread_receiver) =
            std::sync::mpsc::channel::<AudioMessage>();
        let (audio_thread_sender, owned_thread_receiver) = std::sync::mpsc::channel::<i32>();
        ...
        ThreadPool::audio().lock().unwrap().spawn(move || {
            ...
            loop {
                match audio_thread_receiver.try_recv() {
                    Ok(audio_message) => match audio_message {
                        AudioMessage::LoopDestroyed => {
                            break;
                        }
                    },
                            ...
                            while audio_device.get_buffer_len() < 1024 * 8 {
                                match audio_player_item.try_recv() {
                                    Ok(frame) => {
                                        ...
                                        let channel_data_0: &[f32] =
                                            pcm_buffer.get_channel_data_view(0);
                                        let channel_data_1: &[f32] =
                                            pcm_buffer.get_channel_data_view(1);
                                        data.clear();
                                        for (first, second) in
            ...
            let _ = audio_thread_sender.send(0);
        });
        SpawnAudioThreadResult {
            owned_thread_receiver,
            owned_thread_sender,
        }
    }
    ...
    fn spawn_render_thread(
    ...
    ) -> SpawnRenderThreadResult {
        let (owned_thread_sender, render_thread_receiver) =
            std::sync::mpsc::sync_channel::<RenderMessage>(2);
        let (render_thread_sender, owned_thread_receiver) = std::sync::mpsc::channel::<i32>();
        let mut screen_descriptor = Self::get_screen_descriptor(&window);
        ...
        ThreadPool::render().lock().unwrap().spawn({
            move || {
                ...
                loop {
                    let recv_result = render_thread_receiver.recv();
                    match recv_result {
                        Ok(render_message) => match render_message {
                            RenderMessage::LoopDestroyed => break,
                            RenderMessage::RedrawRequested(full_output) => {
                                match wgpu_context.get_current_surface_texture() {
                                    Ok(surface) => {
                ...
                let _ = render_thread_sender.send(0);
            }
        });
        SpawnRenderThreadResult {
            owned_thread_sender,
    ...
    pub fn run(mut self) {
        ...
        let spawn_render_thread_result = Self::spawn_render_thread(
            &self.native_window.window,
            self.egui_context.get_platform_context(),
        );

        let spawn_audio_thread_result = Self::spawn_audio_thread();
        let main_thread_sender = spawn_render_thread_result.owned_thread_sender;
        let main_thread_receiver = spawn_render_thread_result.owned_thread_receiver;
                ...
                match &event {
                    ...
                    WindowEvent { event, .. } => match event {
                        ...
                        winit::event::WindowEvent::Resized(size) => {
                            let _ = main_thread_sender.send(RenderMessage::Resized(*size));
                        }
                        _ => {}
                    },
                    DeviceEvent { .. } => {}
                    ...
                    RedrawRequested(_) => {
                        ...
                        let _ = main_thread_sender
                            .send(RenderMessage::RedrawRequested(Some(full_output)));
                    }
                    RedrawEventsCleared => {
                        self.native_window.window.request_redraw();
                    }
                    LoopDestroyed => {
                        let _ = main_thread_sender.send(RenderMessage::LoopDestroyed);
                        let _ = main_thread_receiver.recv();
                        let _ = spawn_audio_thread_result
                            .owned_thread_sender
                            .send(AudioMessage::LoopDestroyed);
                        let _ = spawn_audio_thread_result.owned_thread_receiver.recv();
                    }
                }
            }
        });
