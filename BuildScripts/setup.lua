...
do
    ...
    on_run(function()
        ...
        local function setup_dotnet(build_type)
            local target_dir = path.join(rs_target_dir, build_type)
            os.mkdir(target_dir)
            os.cp(path.join(dotnet_sdk_dir, "packs/Microsoft.NETCore.App.Host.win-x64/8.0.6/runtimes/win-x64/native/nethost.dll"), target_dir, {copy_if_different = true})
        end
        ...
        local multi_arch_sysroot_lib = path.join(ndk_dir, "toolchains/llvm/prebuilt/windows-x86_64/sysroot/usr/lib")
        os.cp(path.join(deps_dir, "ffmpeg_android/arm64-v8a/lib/*.so"), arch_arm64_v8a_dir, {copy_if_different = true})
        os.cp(path.join(deps_dir, "ffmpeg_android/x86_64/lib/*.so"), arch_x86_64_dir, {copy_if_different = true})
        os.cp(path.join(multi_arch_sysroot_lib, "x86_64-linux-android", "libc++_shared.so"), arch_x86_64_dir, {copy_if_different = true})
        os.cp(path.join(multi_arch_sysroot_lib, "aarch64-linux-android", "libc++_shared.so"), arch_arm64_v8a_dir, {copy_if_different = true})
    end)
    set_menu {
