...
do
    on_run(function()
        ...
        os.exec("cargo build --package rs_editor --bin rs_editor --features editor --features renderdoc --features plugin_shared_crate_export --features plugin_dotnet --features plugin_v8")
        os.exec("cargo build --package rs_editor --bin rs_editor --features editor --features renderdoc --features plugin_shared_crate_export --features plugin_dotnet --features plugin_v8 --release")
        os.exec("./target/release/rs_editor.exe -c")
        os.cd(path.join(engine_root_dir, "rs_desktop_standalone"))
        os.exec("cargo build --package rs_desktop_standalone --bin rs_desktop_standalone --features standalone --features plugin_shared_lib --features plugin_v8")
        os.exec("cargo build --package rs_desktop_standalone --bin rs_desktop_standalone --features standalone --features plugin_shared_lib --features plugin_v8 --release")
